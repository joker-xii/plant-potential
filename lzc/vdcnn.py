...
from lzc.config import BATCH_SIZE
...
class vdcnn(nn.Module):
    def __init__(self, vocab_size, num_labels):
        ...
        self.conv = nn.Conv1d(vocab_size, 64, KERNEL_SIZE, STRIDE, PADDING)
    ...
    def forward(self, x, batch_sz=BATCH_SIZE):
        ...
        h = h.topk(self.k)[0].view(batch_sz, -1)  # k-max pooling
        # print(h.size())
...
class nb_vdcnn(nn.Module):
    def __init__(self, vocab_size, num_labels):
        ...
        self.conv = nn.Conv1d(vocab_size, 64, KERNEL_SIZE, STRIDE, PADDING)
        self.res_blocks = nn.Sequential(  # residual blocks
            res_block(64, 64, bnorm=True),
            res_block(64, 64, "vgg", bnorm=True),
            res_block(64, 128, bnorm=True),
            res_block(128, 128, "vgg", bnorm=True),
            res_block(128, 256, bnorm=True),
            res_block(256, 256, "vgg", bnorm=True),
            res_block(256, 512, bnorm=True),
            res_block(512, 512, bnorm=True)
        )
        self.fc = nn.Sequential(  # fully connected layers
            nn.Linear(512 * self.k, 2048),
            nn.ReLU(),
...
class res_block(nn.Module):  # residual block
    def __init__(self, in_channels, out_channels, downsample=None, bnorm=False):
        super().__init__()
